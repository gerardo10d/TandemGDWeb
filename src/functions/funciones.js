function NadmErosion(TipoEje, CargaxFSC, k, h, Bermas, Dovelas) {
  ...
  if (C1 * P > 9) {
    resultado = Math.round(
      10 ** (14.524 - Math.log(C2, 10) - 6.777 * (C1 * P - 9) ** 0.103)
    );
  } else if (C1 * P <= 9) {
...
function NadmFatiga(tipoEje, CargaxFSC, MR, k, h, bermas){
	...
	E=4*10**6 //psi
	u=0.15 //Relación de Poisson
	CV=0.15 //Coeficiente de variación
	f3=0.894
	f4=1/(1.235*(1-CV))

	ele = (E * (h**3)/12/k/(1-u**2))**0.25 //Radio de rigidez relativa

	if bermas == 0: // 0 es No en la variable del RadioButton

		if tipoEje == 1:
			Med = -1600 + 2525 * math.log(ele,10) + 24.42 * ele + 0.204 * ele ** 2
			f1 = 24 / 18 * (CargaxFSC / 24)**0.94
		else if tipoEje == 2:
			Med = 3029 - 2966.8 * math.log(ele,10) + 133.69 * ele - 0.0632 * ele ** 2
	...
	else if bermas == 1: // 1 es Sí en la variable del RadioButton

		if tipoEje == 1:
			Med = (-970.4 + 1202.6 * math.log(ele,10) + 53.587 * ele) * (0.8742 + 0.01088 * k ** 0.447)
			f1 = 24 / 18 * (CargaxFSC / 24) ** 0.94
			f2 = 1
		else if tipoEje == 2:
			Med = (2005.4 - 1980.9 * math.log(ele,10) + 99.008 * ele) * (0.8742 + 0.01088 * k ** 0.447)
			f1 = 48 / 36 * (CargaxFSC / 48) ** 0.94
			f2 =1
		else if tipoEje == 3:
	...
	if EsfEq / MR >=0.55:
		...
		resultado = round(10 ** (11.737 - 12.077 * (EsfEq / MR)), 0)

	else if 0.45 < EsfEq / MR < 0.55:

		resultado = round((4.2577 / (EsfEq / MR - 0.4325)) ** 3.268, 0)
...
let TipoEje, CargaxFSC, k, h, Bermas, Dovelas;
TipoEje = 1;
CargaxFSC = 36;
k = 130;
h = 9.5;
