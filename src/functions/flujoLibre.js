function flujoLibre(caudal, cotaSup, cotaInf, longitud, tuberiaPVC){

    // Tablas de tuberías y relaciones hidráulicas que verifica la función
    const tablaTuberiasS8 = [[110, 99], [160, 145], [200, 182], [250, 227], [315, 284], [355, 327], [400, 362], [450, 407], [500, 452]];
    
    const tablaTuberiasConcreto = [[6, 152], [8, 203], [10, 254], [12, 305], [15, 381], [18, 457], [24, 610]];
    ...
    let n, tuberias, limiteSanitario
    // Aquí toca decidir si es de PVC o de concreto y seguir
    if (tuberiaPVC) {  // PVC
        n = 0.009
        tuberias = tablaTuberiasS8
        limiteSanitario = 170
    } else {  // Concreto
        n = 0.013
        tuberias = tablaTuberiasConcreto
        limiteSanitario = 140
    }

    let Dinicialm = 1.548 * (n * caudal / 1000 / (pendiente / 100) ** 0.5) ** 0.375  // Diámetro inicial en m
    let Dinicialmm = Math.round(Dinicialm * 1000).toFixed(1)  // Diámetro inicial en mm
    // Filtrar el menor de los que son superiores al Dinicial:
    let Dinterno = Math.min(...tuberias.map(row => row[1]).filter(value => value >= Dinicialmm)); // mm
    // Encontrar la fila donde está el Dinterno en la matriz de tuberías
    //filaDnominal = np.where(tuberias == Dinterno)[0][0]
    let filaDnominal = tuberias.findIndex(element => element === Dinterno);
    
    let Dnominal = tuberias[filaDnominal][0]
    ...
    let Qo = 0.312 * ((Dinterno / 1000) ** (8 / 3)) * ((pendiente / 100) ** (1 / 2)) / n * 1000  // L/s
    let Ao = Math.PI * ((Dinterno / 1000) ** 2) / 4  // m2
    let Vo = Qo / 1000 / Ao  // m/s
    let Ro = Dinterno / 4000  // m
    // print(f'\nCondiciones a tubo lleno:\nQo: {round(Qo, 2)} L/s, '
    //       f'Ao: {round(Ao, 3)} m2, Vo: {round(Vo, 3)} m/s, Ro: {round(Ro, 5)} m')

    // Relaciones hidráulicas
    let QQo = Math.round(caudal / Qo).toFixed(2);
    // Encontrar la columna donde está el Q/Qo en la matriz de relaciones hidráulicas
    let columnaQQo = relHid.findIndex(row => row === QQo);
    let VVo = relHid[1][columnaQQo]
    let dDo = relHid[2][columnaQQo]
    let RRo = relHid[3][columnaQQo]
    ...
    // Fuerza tractiva
    let ft = pesoEspAgua * Rh * pendiente / 100  // Pa
    ...
    ' m2, Vo: ' + Vo.toFixed(3) + ' m/s, Ro: ' + Ro.toFixed(5) + ' m' +
    '\n\nRelaciones hidráulicas:' +
    '\nQ/Qo: ' + QQo + ', V/Vo: ' + VVo + ', D/Do: ' + dDo + ', R/Ro: ' + RRo +
